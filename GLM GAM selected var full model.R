...
CownA.gam <- gam(PA~  s(bio04) + 
                   s(bio12) + s(bio15) +s(aspect),
                 family= binomial, data=Cown.abs)
summary(CownA.gam)
...
CownP.gam<- gam(PA~ s(bio03)+s(bio12)  + s(bio15), 
                family=binomial, data=Cown.pse)
summary(CownP.gam)
...
LownA.gam<- gam(PA~s(bio14)+ s(bio01)+ s(bio04) + s(bio12)              , 
                family= binomial, data=Lown.abs)
summary(LownA.gam)
T2g<- gam(PA~s(bio03)+ s(bio01)+ s(bio04) , 
          family= binomial, data=Lown.abs)
summary(T2g)
...
LownP.gam<- gam(PA~  s(bio04) + s(bio12) + s(rri), 
...
LepiA.gam <- gam(PA~ s(bio01) + s(bio04)+
                   s(bio14) + s(bio15) + te(bio01,bio14), 
                 family= binomial, data=Lepi.abs)
summary(LepiA.gam)
T1g<- gam(PA~ s(bio01) + s(bio04), 
             family= binomial, data=Lepi.abs)
summary(T1g)
P1g<- gam(PA~s(bio14) + s(bio15), 
          family= binomial, data=Lepi.abs)
summary(P1g)
...
summary(LepiP.gam)
T1h<-  gam(PA~ s(bio01) + s(bio04)+ s(bio03), 
           family= binomial, data=Lepi.pse)
P1h<- gam(PA~ s(bio12)+ s(bio14) + s(bio15), 
          family= binomial, data=Lepi.pse)
summary(P1h)
...
# (bio01, bio03, bio04, bio12, bio14, bio15, rri, slope, aspect, and eq)
# R. cowanianum with Rhododendron absence
CownA.glm<- glm(PA~ bio04  +bio14+  bio12 + slope , 
                family=binomial, data = Cown.abs)
summary(CownA.glm)
...
# R. cowanianum with Random Pseudo absence
CownP.glm<- glm(PA~bio01+I(bio01^2) + bio03 + I(bio03^2)+ bio14,
                family=binomial, data = Cown.pse)
summary(CownP.glm)
(1-CownP.glm$deviance / CownP.glm$null.deviance)
T3b <- glm(PA~bio01+I(bio01^2) + bio03 + I(bio03^2),
                       family=binomial, data = Cown.pse)
summary(T4a)
...
# R. lowndesii with Rhododendron absence
LownA.glm<- glm(PA ~ bio03 + I(bio03^2)+ bio04 + I(bio04^2) + 
                  bio14+ I(bio14^2) + aspect, 
                family=binomial, data=Lown.abs)
summary(LownA.glm)
...
# R. lowndesii with Random Pseudo absence
LownP.glm<- glm(PA~bio04+I(bio04^2) + bio03+I(bio03^2)+
                  bio14+ I(bio14^2) + eq + bio04:bio14,
                family= binomial, data = Lown.pse)
summary(LownP.glm)
(1-LownP.glm$deviance / LownP.glm$null.deviance)
T2b<- glm(PA~bio04+I(bio04^2) + bio03+I(bio03^2) ,
          family= binomial, data = Lown.pse)
summary(T2b)

P2b<- glm(PA~bio14+ I(bio14^2),
          family= binomial, data = Lown.pse)
summary(P2b)
...
# R. lepidotum with Rhododendron absence
LepiA.glm<- glm(PA~bio01+ I(bio01^2) + bio03 + I(bio03^2)+
                bio15  + rri + aspect, 
                family= binomial, data=Lepi.abs)
summary(LepiA.glm)
